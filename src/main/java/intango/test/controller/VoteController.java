...
public class VoteController {
	...
	@PostMapping("/vote/{colorId}")
	public ResponseEntity<?> voteFor(@PathVariable int colorId) {

		try {

			colorVoteService.voteFor(colorId);;
			return ResponseEntity.ok(colorId);
		} catch (VoteException e) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("Id doesnt exists");
		}
